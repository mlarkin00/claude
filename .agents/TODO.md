...
- [ ] **[P2]** `agent-memory/README.md` links to a private repo that 404s for anyone else — "GitHub repo ([agent-memory](https://github.com/mlarkin00/agent-memory))" at lines 5 and 100. *(2026-07-23: the stale-install-instructions half of this item was fixed during the agents→skills refactor — the marketplace-add line now names `mlarkin00/plugins` and install carries the `@mlarkin00-plugins` qualifier. The private-repo links remain.)*
