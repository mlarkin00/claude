---
name: gcloud
description: Use this skill when interacting with Google Cloud services using the gcloud CLI. Use when managing cloud resources, querying configurations, or troubleshooting issues via gcloud.
metadata:
  category: infra-ops
---
...
If the `gcloud` executable is missing, refer to the official [Google Cloud CLI Installation Guide](https://docs.cloud.google.com/sdk/docs/install-sdk) to install it on your platform (Linux, macOS, Windows, etc.).
...
For more detailed strategies and authentication types (such as Workload Identity Federation), see [Authorizing the gcloud CLI](https://docs.cloud.google.com/sdk/docs/authorizing).
...
*   **Forgetting `--project`**: Commands run without `--project` may target the wrong active configuration, leading to errors or unintended resource modification.
*   **Prompting for Location**: Omitting `--region` or `--zone` often triggers an interactive prompt, which causes agents to hang or fail.
*   **Unconstrained Lists**: Running `list` without `--limit` or `--filter` in a large environment can crash the agent session by exhausting the context window.
*   **Assuming APIs are Enabled**: `gcloud` will fail if the underlying API is not enabled. Check API status if commands fail with 'permission denied' or 'api not enabled' errors.
*   **Quoting Filters**: Incorrectly quoting `--filter` expressions often leads to syntax errors. Treat the entire filter as a single string.
...
## Red Flags (Shortcuts to Avoid)

*   Skipping `gcloud help` before execution.
*   Using `| grep` instead of `--filter`.
*   Using `| awk` or `| jq` instead of `--format`.
*   Running destructive commands (`delete`, `remove`) without explicit user permission.
*   Executing multiple `gcloud` commands in a single shell string.
...
Refer to the [gcloud CLI Scripting Guide](https://docs.cloud.google.com/sdk/docs/scripting-gcloud) for guidance on using the gcloud CLI in automation.
